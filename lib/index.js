...
async function createPullRequest(
  ...
  console.log(`Pull request #${pr_number} (${url}) has been opened`);

  if (Array.isArray(labels) && labels.length > 0) {
    console.log(`Adding labels [${labels.join(', ')}] to Pull request #${pr_number}`);
    // Pull requests are just fancy issues
    await oktokit.issues.addLabels({
      owner,
      repo,
